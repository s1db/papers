...
% SAT clause nodes
\node[sat] (S1) at (0.0, -1.5) {$e_1 \vee e_3^{u\leftarrow 0}$};
\node[sat] (S2) at (2.8, -1.5) {$\neg e_1 \vee e_3^{u\leftarrow 1}$};
\node[sat] (S3) at (5.2, -1.5) {$\neg e_3^{u\leftarrow 0}$};
\node[sat] (S4) at (7.6, -1.5) {$\neg e_3^{u\leftarrow 1}$};
...
% Refutation nodes
\node[res] (R1) at (1.5, -3) {$e_3^{u\leftarrow 0} \vee e_2^{u\leftarrow 1}$};
\node[res] (R2) at (4.5, -4) {$e_3^{u\leftarrow 1}$};
...
\iffalse
    ...
    Expanding $e$; universal variable $b$ becomes annotated copies $b^{e\leftarrow 0}$, $b^{e\leftarrow 1}$:
    \[
        \psi_\exists \;=\;
        \underbrace{\bigl[(\neg a \vee \neg b^{e\leftarrow 1}) \wedge (\neg a \vee b^{e\leftarrow 1})\bigr]}_{\phi[e/1]}
        \;\vee\;
        \underbrace{\bigl[(a \vee b^{e\leftarrow 0})\bigr]}_{\phi[e/0]}
    \]
    ...
    \[
        \neg\psi_\exists \;=\;
        \underbrace{\bigl[(a \wedge b^{e\leftarrow 1}) \vee (a \wedge \neg b^{e\leftarrow 1})\bigr]}_{\neg\phi[e/1]\;\text{(DNF)}}
        \;\wedge\;
        \underbrace{(\neg a \wedge \neg b^{e\leftarrow 0})}_{\neg\phi[e/0]\;\text{(DNF)}}
    \]
    ...
    \smallskip
    \begin{tabular}{lll}
        Branch $e \leftarrow 1$: & $H_2 = (\neg t_2 \vee a)(\neg t_2 \vee b^{e\leftarrow 1})$ & $N_{e\leftarrow 1} = (t_2 \vee t_3)$ \\
                                  & $H_3 = (\neg t_3 \vee a)(\neg t_3 \vee \neg b^{e\leftarrow 1})$ & \\[4pt]
        Branch $e \leftarrow 0$: & $H_1 = (\neg t_1 \vee \neg a)(\neg t_1 \vee \neg b^{e\leftarrow 0})$ & $N_{e\leftarrow 0} = (t_1)$ \\
    \end{tabular}
    
    \medskip
    \textbf{Resulting CNF passed to SAT solver:}
    \[
        \psi_F \;=\; \underbrace{(t_2 \vee t_3)}_{N_{e\leftarrow 1}} \wedge \underbrace{(t_1)}_{N_{e\leftarrow 0}} \wedge H_1 \wedge H_2 \wedge H_3
    \]
...
\begin{column}{0.65\textwidth}
    ...
    \begin{align*}
        ...
        \onslide<4-5>{\text{assignment} &= \{e_2\leftarrow 0, e_5\leftarrow 0, e_6\leftarrow 0\} \quad \checkmark}\\
    ...
    \begin{align*}
        ...
        \onslide<9-10>{\text{assignment} &= \{e_2\leftarrow 0, e_5\leftarrow 1, e_6\leftarrow 0\} \quad \checkmark}\\
